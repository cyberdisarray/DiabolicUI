...
Module.GrabKeybinds = Module:Wrap(function(self)
	...
	if not self.binding_table then
		...
		if ENGINE_CATA then
			table_insert(self.binding_table, "EXTRAACTIONBUTTON%d") -- extra action button
		end
	end
	...
	RegisterStateDriver(self.vehicle_controller, "vehicle", ENGINE_MOP and "[overridebar][possessbar][shapeshift][vehicleui]vehicle;novehicle" or "[bonusbar:5][vehicleui]vehicle;novehicle")
end)
...
Module.OnInit = function(self, event, ...)
	...
	local side = self:GetWidget("Controller: Side")
	side:Enable()

	-- Set the keywords for our main and side controllers, 
	-- as both the other widgets and other modules rely on them. 
	-- We upvalue them for faster reference we know they won't change.
	local Main = main:GetFrame()
	local Side = side:GetFrame()

	Engine:RegisterKeyword("Main", function() return Main end)
	Engine:RegisterKeyword("Side", function() return Side end)
	...
	self.bars = {} 
	table_insert(self.bars, self:GetWidget("Bar: 1"):GetFrame())	-- 1
	table_insert(self.bars, self:GetWidget("Bar: 2"):GetFrame())	-- 2
	table_insert(self.bars, self:GetWidget("Bar: 3"):GetFrame())	-- 3
	table_insert(self.bars, self:GetWidget("Bar: 4"):GetFrame())	-- 4
	table_insert(self.bars, self:GetWidget("Bar: 5"):GetFrame())	-- 5
	table_insert(self.bars, self:GetWidget("Bar: Pet"):GetFrame()) -- 7
	table_insert(self.bars, self:GetWidget("Bar: Stance"):GetFrame()) -- 6
	--if ENGINE_CATA then
		--table_insert(self.bars, self:GetWidget("Bar: Extra"):GetFrame()) --8
	--end
