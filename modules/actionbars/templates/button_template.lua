...
local colors = {
	...
	out_of_range = C.Status.OutOfRange, -- spell target is out of range (too far / too close)
	out_of_mana = C.Status.OutOfMana, -- player has too little mana

	-- icon overlays
	unusable_overlay = { C.General.UIOverlay[1], C.General.UIOverlay[2], C.General.UIOverlay[3], .25 }, 
	flight_overlay = { C.General.UIOverlay[1], C.General.UIOverlay[2], C.General.UIOverlay[3], .7 }, 
	range_overlay = { C.General.UIOverlay[1], C.General.UIOverlay[2], C.General.UIOverlay[3], .10 }, 

	stack_text = { 1, 1, 1, 1 },
	name_text = { 1, 1, 1, 1 },
	keybind_text = { 1, 1, 1, 1 },
	keybind_text_disabled = { .3 + .5* C.Status.Dead[1], .3 + .5* C.Status.Dead[2], .3 + .5* C.Status.Dead[3] },
...
local OnUpdate = function(_, elapsed)
	...
	if rangeTimer <= 0 or flashTime <= 0 then
		...
		if rangeTimer <= 0 then
			rangeTimer = 1/20 -- TOOLTIP_UPDATE_TIME (0.2)
		end
	end
end
...
Button.GetKeyBind = function(self)
	local key = self.keybindAction and GetBindingKey(self.keybindAction) or GetBindingKey("CLICK "..self:GetName()..":LeftButton")
...
Button.UpdateUsable = function(self)
	if UnitOnTaxi("player") or IsFlying() then
		...
		if self.icon:SetDesaturated(true) then
			self.icon:SetVertexColor(colors.usable[1], colors.usable[2], colors.usable[3])
			self.icon.dark:SetVertexColor(colors.flight_overlay[1], colors.flight_overlay[2], colors.flight_overlay[3], colors.flight_overlay[4])
			self.icon.dark:Show()
		else
			-- fallback to standard darkening if desaturation fails
			self.icon:SetDesaturated(false)
			self.icon:SetVertexColor(colors.unusable[1], colors.unusable[2], colors.unusable[3])
			self.icon.dark:SetVertexColor(colors.unusable_overlay[1], colors.unusable_overlay[2], colors.unusable_overlay[3], colors.unusable_overlay[4])
			self.icon.dark:Show()
		end
	else
		local isUsable, notEnoughMana = self:IsUsable()
		local outOfRange = self.outOfRange or not(self:IsInRange())
		...
		elseif outOfRange then
			self.icon:SetDesaturated(false)
			self.icon:SetVertexColor(colors.out_of_range[1], colors.out_of_range[2], colors.out_of_range[3])
			self.icon.dark:SetVertexColor(colors.range_overlay[1], colors.range_overlay[2], colors.range_overlay[3], colors.range_overlay[4])
			self.icon.dark:Show()
		...
		elseif notEnoughMana then
			self.icon:SetDesaturated(false)
			self.icon.dark:Hide()
			self.icon:SetVertexColor(colors.out_of_mana[1], colors.out_of_mana[2], colors.out_of_mana[3])
		else
			self.icon:SetDesaturated(false)
			self.icon:SetVertexColor(colors.unusable[1], colors.unusable[2], colors.unusable[3])
			self.icon.dark:SetVertexColor(colors.unusable_overlay[1], colors.unusable_overlay[2], colors.unusable_overlay[3], colors.unusable_overlay[4])
...
ActionButton.GetCharges              = function(self) 
	return (ENGINE_MOP and GetActionCharges(self.action_by_state))
end
...
ActionButton.IsConsumableOrStackable = function(self) 
	return IsConsumableAction(self.action_by_state) 
		...
		or (ENGINE_MOP and (not IsItemAction(self.action_by_state) and GetActionCount(self.action_by_state) > 0))
end
...
ButtonWidget.New = function(self, buttonType, id, header)
	...
	--button.icon:SetTexCoord(5/64, 59/64, 5/64, 59/64) -- don't need it with an inner shade?

	--button.icon.darken = button:CreateTexture(nil, "OVERLAY")
	--button.icon.darken:SetAllPoints(button.icon)
	--button.icon.darken:SetColorTexture(0, 0, 0, .1)

	local path = ([[Interface\AddOns\%s\media\]]):format(ADDON) 
	local auraShade = path .. [[textures\DiabolicUI_Shade_64x64.tga]]
	button.icon.shade = button:CreateTexture(nil, "OVERLAY")
	button.icon.shade:SetAllPoints(button.icon)
	button.icon.shade:SetTexture(auraShade)
	button.icon.shade:SetVertexColor(0, 0, 0, .75)
