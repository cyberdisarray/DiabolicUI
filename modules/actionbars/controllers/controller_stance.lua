...
ControllerWidget.OnEnable = function(self)
	...
	self.Controller = Engine:CreateFrame("Frame", nil, "UICenter", "SecureHandlerAttributeTemplate")
	self.Controller:SetFrameStrata("BACKGROUND")
	self.Controller:Place(point, anchor, anchor_point, xoffset, yoffset)
...
ControllerWidget.UpdateStanceButton = Engine:Wrap(function(self)
	...
	if num_forms == 0 then
		...
		UnregisterStateDriver(Controller, "visibility")
		RegisterStateDriver(Controller, "visibility", ENGINE_MOP and "[overridebar][possessbar][shapeshift][vehicleui]hide;show" or "[bonusbar:5][vehicleui]hide;show")
	end
